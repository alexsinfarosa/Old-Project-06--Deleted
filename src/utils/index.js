...
export const getData = async (
  ...
  let acis = [];
  let days = 6;
  if (subject.name === "Blueberry Maggot") {
    days = 5;
  }
  ...
  for (const [i, day] of results.entries()) {
    ...
    if (isTpClean) {
      ...
      results[i]["Tmin"] = Tmin.toFixed(0);
      results[i]["Tmax"] = Tmax.toFixed(0);
      results[i]["Tavg"] = Tavg.toFixed(0);
      ...
      const dd = Math.round(baskervilleEmin(Tmin, Tmax, base));
      results[i]["dd"] = dd;
